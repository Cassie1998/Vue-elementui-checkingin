    ...
    <div>
        <el-form :model="form" :rules="rules2" ref="form" label-position="left" label-width="0px" class="demo-ruleForm card-box loginform">
            ...
            <el-form-item style="width:100%;">
                <el-button type="primary" style="width:100%;" v-loading="loading" @click.native.prevent="handleSubmit2('form')">签到</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
                    ...
                    if (valid) {
                        ...
                        this.apiPost('admin/base/login', data).then((res) => {
                            if (res.code != 200) {
                                ...
                                _g.toastMsg('success', '签到成功')
                                setTimeout(() => {
                                    router.replace('/Qiansuccess')
                                }, 1500)
                            }
                        })
                    } else {
